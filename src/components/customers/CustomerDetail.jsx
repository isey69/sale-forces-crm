...
const CustomerDetail = memo(() => {
  ...
  const handleRelationshipSearch = useCallback(async () => {
    ...
    try {
      const results = await customerService.searchCustomersByName(
  ...
  const handleCancelScheduledCall = useCallback(
    async (callId) => {
      try {
      ...
      <div className="max-w-7xl mx-auto">
                      ...
                      {applicableCustomFields.map((field) => (
                        <div key={field.id} className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-gray-900">
                              {customer.customFields?.[field.name] || "N/A"}
                            </p>
                          </div>
                        </div>
                      ))}
              ...
              <div>
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-sm text-gray-600">
                                  {call.date instanceof Date
                                    ...
                                    : call.date}{" "}
                                  at {call.time} • Duration: {call.duration}
                                </p>
                              </div>
                              <div className="flex items-center gap-2">
                                <span
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      {scheduledCalls &&
                      scheduledCalls.filter((c) => c.status !== "completed")
                        .length > 0 ? (
                        scheduledCalls
                            ...
                            >
                              <div className="flex items-start justify-between mb-3">
                                <div className="flex-1">
                                  ...
                                  <p className="text-sm text-gray-600">
                                    {call.scheduledDate instanceof Date
                                      ? call.scheduledDate.toLocaleDateString()
                                      : call.scheduledDate}{" "}
                              ...
                              <div className="flex justify-end mt-3 gap-2">
                                ...
                                <button
                                  onClick={() =>
                                    handleCancelScheduledCall(call.id)
                                  }
          ...
          <div className="p-2">
            <div className="flex items-center gap-2 mb-4">
              <input
                ...
                onKeyPress={(e) =>
                  e.key === "Enter" && handleRelationshipSearch()
                }
        ...
        >
          <form onSubmit={handleCallFormSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Link to Scheduled Call (Optional)
              </label>
              <select
                onChange={(e) => {
                  const callId = e.target.value;
                  if (callId) {
                    const selectedCall = scheduledCalls.find(
                      (c) => c.id === callId
                    );
                    if (selectedCall) {
